...
function makeNtlmRequest(documentNo: string, approvalStatus: any, approvalLevel: number): Promise<Response> {
...
export async function POST(req: NextRequest): Promise<Response> {
  try {
    ...
    const { documentNo, approvalStatus, approvalLevel } = body;

    if (!documentNo) {
      return NextResponse.json({ error: "Missing documentNo" }, { status: 400 });
    }

    return await makeNtlmRequest(documentNo, approvalStatus, approvalLevel);
  } catch (error: any) {
